...
def test_can_access_settings_while_not_authenticated(client):
    response = client.get(url_for("settings.settings"), follow_redirects=False)
...
def test_writing_settings_to_file_while_not_authenticated(client):
    response = client.post(url_for("settings.export"), follow_redirects=False)
...
def test_attempting_to_upload_while_not_authenticated(client):
    response = client.post(url_for("settings.upload_settings"), follow_redirects=False)
...
def test_attempting_to_upload_valid_settings_file(app, auth, client, settings, user):
    ...
    data = {
        "file": FileStorage(stream=open(json_file, "rb"), filename=json_file),
        "submit": True,
    }
    with app.test_request_context():
        auth.login()
        response = client.post(
            url_for("settings.upload_settings"), data=data, follow_redirects=False
        )
